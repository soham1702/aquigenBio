...
class SecurityUserResourceIT {
    ...
    @Test
    @Transactional
    void getAllSecurityUsersByPurchaseOrderIsEqualToSomething() throws Exception {
        // Initialize the database
        securityUserRepository.saveAndFlush(securityUser);
        ...
        em.persist(purchaseOrder);
        em.flush();
        securityUser.addPurchaseOrder(purchaseOrder);
        securityUserRepository.saveAndFlush(securityUser);
        Long purchaseOrderId = purchaseOrder.getId();

        // Get all the securityUserList where purchaseOrder equals to purchaseOrderId
        defaultSecurityUserShouldBeFound("purchaseOrderId.equals=" + purchaseOrderId);

        // Get all the securityUserList where purchaseOrder equals to (purchaseOrderId + 1)
        defaultSecurityUserShouldNotBeFound("purchaseOrderId.equals=" + (purchaseOrderId + 1));
    }
