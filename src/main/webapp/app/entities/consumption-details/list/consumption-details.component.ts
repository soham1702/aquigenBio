...
@Component({
  selector: 'jhi-consumption-details',
...
export class ConsumptionDetailsComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.consumptionDetailsService
      ...
      .subscribe({
        next: (res: HttpResponse<IConsumptionDetails[]>) => {
  ...
  trackId(index: number, item: IConsumptionDetails): number {
    return item.id!;
  }

  delete(consumptionDetails: IConsumptionDetails): void {
    const modalRef = this.modalService.open(ConsumptionDetailsDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.consumptionDetails = consumptionDetails;
  ...
  protected onSuccess(data: IConsumptionDetails[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/consumption-details'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
