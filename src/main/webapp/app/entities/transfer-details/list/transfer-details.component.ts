...
export class TransferDetailsComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.transferDetailsService
      ...
      .subscribe({
        next: (res: HttpResponse<ITransferDetails[]>) => {
  ...
  trackId(index: number, item: ITransferDetails): number {
    return item.id!;
  }

  delete(transferDetails: ITransferDetails): void {
    const modalRef = this.modalService.open(TransferDetailsDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: ITransferDetails[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
